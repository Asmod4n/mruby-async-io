...
struct cares_ctx {
  ...
  mrb_value errors;
  ares_socket_t dns_client_fds[ARES_GETSOCK_MAXNUM];
  int bitmask;
};
...
mrb_append_error(struct cares_ctx *ctx, int status)
{
  switch (status) {
    case ARES_ENOTIMP:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "notimp")));
    break;
    case ARES_EBADNAME:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "badname")));
    break;
    case ARES_ENODATA:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "nodata")));
    break;
    case ARES_ENOTFOUND:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "notfound")));
    break;
    case ARES_ENOMEM:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "nomem")));
    break;
    case ARES_ECANCELLED:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "cancelled")));
    break;
    case ARES_EDESTRUCTION:
      mrb_ary_push(ctx->mrb, ctx->errors, mrb_symbol_value(mrb_intern_lit(ctx->mrb, "destruction")));
    break;
  }
}
...
mrb_append_ai(struct cares_ctx *cares_ctx, struct RClass *addrinfo_class, struct ares_addrinfo_node *node)
{
  ...
  mrb_value addrinfo = mrb_obj_new(cares_ctx->mrb, addrinfo_class, 4, args);
...
mrb_ares_new(mrb_state *mrb, mrb_value self)
{
  ...
  cares_ctx->errors = mrb_ary_new(mrb);
  mrb_iv_set(mrb, self, mrb_intern_lit(mrb, "@errors"), cares_ctx->errors);
...
mrb_ares_getaddrinfo(mrb_state *mrb, mrb_value self)
{
  ...
  mrb_ary_clear(mrb, cares_ctx->errors);
  ...
  switch (ss.ss_family) {
    ...
    default: {
      mrb_raise(mrb, E_ARGUMENT_ERROR, "socket is neither IPv4 nor IPv6");
    }
  }
...
{
  struct cares_ctx *cares_ctx = DATA_PTR(self);
  int socks[ARES_GETSOCK_MAXNUM] = {0};
  int bitmask = ares_getsock(cares_ctx->channel, socks, ARES_GETSOCK_MAXNUM);
  struct RClass *ares_socket_class = mrb_class_get_under(mrb, mrb_class(mrb, self), "Socket");
  ...
  if (mrb_block_given_p(mrb))  {
    ...
    while ((sock = socks[i])) {
      mrb_value socket   = mrb_fixnum_value(sock);
      mrb_value readable = mrb_bool_value(ARES_GETSOCK_READABLE(bitmask, i));
      mrb_value writable = mrb_bool_value(ARES_GETSOCK_WRITABLE(bitmask, i));
      mrb_value argv[] = {socket, readable, writable};
      mrb_yield(mrb, block, mrb_obj_new(mrb, ares_socket_class, 3, argv));
      mrb_gc_arena_restore(mrb, arena_index);
      i++;
    }
  } else {
    mrb_value sockets = mrb_ary_new(mrb);
    int arena_index = mrb_gc_arena_save(mrb);
    while ((sock = socks[i])) {
      mrb_value socket   = mrb_fixnum_value(sock);
      mrb_value readable = mrb_bool_value(ARES_GETSOCK_READABLE(bitmask, i));
      mrb_value writable = mrb_bool_value(ARES_GETSOCK_WRITABLE(bitmask, i));
      mrb_value argv[] = {socket, readable, writable};
      mrb_ary_push(mrb, sockets, mrb_obj_new(mrb, ares_socket_class, 3, argv));
      mrb_gc_arena_restore(mrb, arena_index);
      i++;
    }
    return sockets;
...
mrb_mruby_async_io_gem_init(mrb_state* mrb)
{
  ...
  struct RClass *mrb_async_class, *mrb_async_io_class, *mrb_async_ares_class;
  ...
  mrb_async_io_class = mrb_define_class_under(mrb, mrb_async_class, "IO", mrb->object_class);
  ...
  mrb_define_method(mrb,  mrb_async_ares_class, "initialize",   mrb_ares_new,         MRB_ARGS_REQ(3));
  mrb_define_method(mrb,  mrb_async_ares_class, "getaddrinfo",  mrb_ares_getaddrinfo, MRB_ARGS_REQ(3));
  mrb_define_method(mrb,  mrb_async_ares_class, "getsock",      mrb_ares_getsock,     MRB_ARGS_NONE());
...
void mrb_mruby_async_io_gem_final(mrb_state* mrb)
{
